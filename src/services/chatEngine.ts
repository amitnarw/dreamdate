  ...
  const initialMessages: ChatMessage[] = [
    {
      id: 'msg-init-1',
      sender: 'profile',
      text: `Hii dear! Main ${profileName}. Kaise ho aap?`,
...
export function getSimulatedReply(userMessage: string, profileName: string): { text: string; delayMs: number } {
  ...
  if (lower.includes('call') || lower.includes('video') || lower.includes('live')) {
    replies = [
      ...
      `Abhi video call connect karo na dear, face-to-face baat karte hain.`,
    ...
    replies = [
      ...
      `Hello handsome! Aapse baat karke bohot accha laga. Kahan se ho aap?`,
    ...
    replies = [
      ...
      `Taareef ke liye shukriya! Video call pe aur bhi sundar lagti hoon, aakar dekh lo.`,
    ...
    replies = [
      `Thode se coins lagte hain bas, par trust me aapse baat karke bohot maza aayega!`,
      `Call rate bohot low hai dear, jaldi se recharge karke video call pe aao na!`,
    ];
  } else {
    replies = [
      `Sahi baat hai! Waise aapse typing se zyada bolke baat karna accha lagega. Ek baar call karo na?`,
